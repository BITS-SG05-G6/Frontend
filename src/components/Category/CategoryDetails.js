        ...
        <div className="w-96">
            <div className=" bg-white rounded-[10px] border border-gray-300 flex flex-col gap-5 p-6">
                <div className="px-2 py-3 flex flex-col gap-2 text-center">
                    <Text weight='bold' variant='text-xl'>{category.name}</Text>
                    <Text variant="text-md" weight="semibold" className='text-primary'>{category.amount}</Text>
                    {/* Icon */}
                        ...
                        >
                            {IconList.map((i) =>
                                i.value === category.icon ? (
                                    <FontAwesomeIcon icon={i.icon} size="4x" color={category.color} />
                                ) : null
                            )}
                        </div>
                    </div>
